	...
	if (ok==0), fprintf('Image "%s" can not be resampled\n', st.vols{i}.fname);
		...
		if isfield(st.vols{i},'blobs'),
			...
			elseif isstruct(st.vols{i}.blobs{1}.colour),
				...
				imgt = scaletocmap(imgt,mn,mx,gryc,[1 64 65]);
				imgc = scaletocmap(imgc,mn,mx,gryc,[1 64 65]);
				imgs = scaletocmap(imgs,mn,mx,gryc,[1 64 65]);
				gryc = [gryc; 0 0 0];
				...
				% actimg scaled round mn mx, black NaNs
                                topc = size(actc,1);
				lrn = [1 topc topc+1];
				tmpt = scaletocmap(tmpt,cmn,cmx,actc,lrn);
				tmpc = scaletocmap(tmpc,cmn,cmx,actc,lrn);
				tmps = scaletocmap(tmps,cmn,cmx,actc,lrn);
				actc = [actc; 0 0 0];
