...
%                       ignored) 
% scales              - number of scales for wt
% 
% Outputs 
...
% (scales by ndims) 
%
% The problem is that the wt fpr one scale and one
% dimension may generate a matrix that is larger by 1 in the transformed
% dimension, because the wt will generate a padding 0 at the end before
% doing the wt, so that the ouputs can be decimated.  So we need to be able
% to predict the various sizes as we go through the wt, in order to get and
% set the data to/from the pre-wt and post-wt matrices.
% 
% $Id: wt_dims.m,v 1.1 2004/07/12 22:16:11 matthewbrett Exp $
