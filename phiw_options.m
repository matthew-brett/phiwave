 ...
 case 'edit'
  ...
  switch defarea
   ...
   case 'wt'
    ...
    phiw = getdefs(phiw,class(oldphiw.wt.wavelet),'wt.wavelet',...
			'Wavelet',wtypes,wlabs);
    switch phiw.wt.wavelet
     case 'phiw_lemarie'
      wavelet = phiw_lemarie;
     case 'phiw_daub'
      wavelet = phiw_daub;
    end
    phiw.wt.wavelet = uisetparams(wavelet,oldphiw.wt.wavelet);
   ...
   case 'display'
     ...
     while isempty(cmap)
       ...
       [cmap str] = slice_overlay('getcmap', cmapname);
       if isempty(cmap)
	 disp(str);
       end
     end
